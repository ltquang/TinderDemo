...
class TinderDemoTests: QuickSpec {

    override func spec() {
        let viewController = ViewController()
        describe("get user models") {
            it("get success") {
                viewController.httpService = HttpServiceMockSuccess()
                viewController.favoriteManager = FavoriteManager.shared
                viewController.httpService.getUser { (response) in
                    if let results = response?["results"] as? [[String: Any]] {
                        viewController.userModels.removeAll()
                        for result in results {
                            viewController.userModels.append(UserModel(json: result))
                        }
                        expect(viewController.userModels.count).to(be(1))
                    }
                }
            }
            it("get fail") {
                viewController.httpService = HttpServiceMockFail()
                viewController.favoriteManager = FavoriteManager.shared
                viewController.httpService.getUser { (response) in
                    if let results = response?["results"] as? [[String: Any]] {
                        viewController.userModels.removeAll()
                        ...
                        expect(viewController.userModels.count).to(be(0))
                    }
                }
            }
        }
